...
1. **Parsing determinístico** de PDF com PyMuPDF (preserva página + bbox)
...
```
PDF → Parser (PyMuPDF) → Blocos → FAISS Index
                                      ↓
...
### Exemplo CRI sem Destinação

```bash
python scripts/validate.py --pdf_path docs/exemplo_cri_sem_destinacao.pdf --standard_path standards/CRI/cri_sem_destinacao.v1.json
```
